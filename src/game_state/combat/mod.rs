...
lazy_static! {
    pub static ref MONSTERS: HashMap<String, Monster> = [
    ...
    .collect();
    pub static ref MONSTER_MODIFIERS: HashMap<String, MonsterModifier> = [
    ...
    .into_iter()
    .map(|monster_modifier| (monster_modifier.name.clone(), monster_modifier))
    .collect();
}
...
impl Monster {
    ...
    pub fn spawn(self, modifier: MonsterModifier) -> SpawnedMonster {
        let mut rng = thread_rng();
        let hitpoint_jitter = Uniform::new(1.0 / 1.1, 1.1).sample(&mut rng);
        let currency_jitter = Gamma::new(2.0, 0.25).unwrap().sample(&mut rng) + 0.5;
        SpawnedMonster {
            hitpoints: (hitpoint_jitter * (self.hitpoints as f64) * modifier.hitpoint_factor)
                .round() as u64,
            currency_reward: Currency::from_copper(
                (self.currency_reward.copper() as f64 * currency_jitter).round() as i128,
            ),
            monster: self,
            modifier,
        }
